...
namespace PrintServiceCardApp.Model.Helper
{
    ...
    public class PasswordStorage
    {
        ...
        //public const int SALT_INDEX = 3;
        public const int PBKDF2_INDEX = 0;
        ...
        public static string CreateHash(string password)
        {
            ...
            // format:salt:hash:hashSize
            var part = string.Format("{0}:{1}:{2}", Convert.ToBase64String(salt), Convert.ToBase64String(hash), hash.Length);
            return part;
        }

        public static bool VerifyPassword(string password, string salt, string goodHash)
        {
            char[] delimiter = { ':' };
            ...
            try
            {
                salts = Convert.FromBase64String(salt);
            }
            catch (ArgumentNullException ex)
            {
                throw new CannotPerformOperationException(
